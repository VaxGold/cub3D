...
/*   Updated: 2021/04/07 21:00:56 by omercade         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    raycast_draw(t_raycast *ryc, t_data *gd, t_render *render, int x)
{
	...
	pix = render->drawStart;
	while (pix < render->drawEnd)
	{
		int texY = (int)render->texPos & (gd->tex[ryc->face].height - 1);
		render->texPos += render->step;
		color = gd->tex[ryc->face].addr[gd->tex[ryc->face].height * texY + render->texX];
		gd->cnv.img.data[pix * gd->cnv.w + x] = color;
		pix++;
	}
	pix = render->drawEnd;
	while (pix <= gd->cnv.h)
	{
		gd->cnv.img.data[pix * gd->cnv.w + x] = gd->color[1];
		pix++;
	}
}
...
void    raycast_wall(t_raycast *ryc, t_data *gd, int x)
{
	...
	this.drawStart = -(ryc->lineHeight) / 2 + gd->cnv.h / 2;
	if(this.drawStart < 0)
		this.drawStart = 0;
	this.drawEnd = ryc->lineHeight / 2 + gd->cnv.h / 2;
	...
	this.texPos = (this.drawStart - gd->cnv.h / 2 + ryc->lineHeight / 2) * this.step;
...
double    ft_raycaster(t_data *gd, int x)
{
	...
	raycast_hit(&this, gd);
	if (this.side == 0)
		this.perpWallDist = (this.mapX - gd->actor.posX + (1 - this.stepX) / 2) / this.dirX;
	else
		this.perpWallDist = (this.mapY - gd->actor.posY + (1 - this.stepY) / 2) / this.dirY;
